---
description: "Scaffold the .specify/rules/ business rules graph for this project."
scripts:
  sh: ../../scripts/bash/rules-init.sh
  ps: ../../scripts/powershell/rules-init.ps1
---
...
2. Report the created paths back to the user (`rules_dir`, `extension_dir`, `initialized`).
...
A one-line confirmation listing the rules directory and a hint to run `/speckit.byte-sized.add` to capture the first rule.
