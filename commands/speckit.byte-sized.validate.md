...
Quick integrity check of the rules graph. Cheaper than `analyze` — no coverage or conflict scan.
...
3. If `ok == true` and `warnings == []`, print `Rules graph is healthy.` and exit.
