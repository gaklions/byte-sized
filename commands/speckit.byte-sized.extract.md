---
description: "Extract candidate business rules from a spec or clarification log into the staging area for review."
scripts:
  sh: ../../scripts/bash/rules-extract.sh
  ps: ../../scripts/powershell/rules-extract.ps1
---

# /speckit.byte-sized.extract

Propose candidate business rules from a source document and stage them for the user to review.
...
2. **Read the source file** and propose candidate rules. Each candidate must be a single sentence in declarative form ("must / must not / always / never"), with a guess at `domain` (from the configured domain list — see `byte-sized-config.yml`) and 3–6 lowercase `tags`. Avoid duplicating phrasing already covered by existing rules (run `/speckit.byte-sized.query --text "<key phrase>"` first if the spec is large).
...
5. The script de-duplicates against the existing index and writes survivors to `.specify/rules/_drafts/<feature>-<timestamp>.yml`. It prints `{drafts_file, kept, dropped_as_duplicate}`.
...
A short summary (counts) plus the list of new rule ids if any drafts were promoted.
