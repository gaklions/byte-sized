---
description: "Brownfield baseline pass: sweep the existing codebase and stage candidate business rules for review."
scripts:
  sh: ../../scripts/bash/rules-baseline.sh
  ps: ../../scripts/powershell/rules-baseline.ps1
---

# /speckit.byte-sized.baseline

One-shot adoption command. Sweep the existing repo (docs, specs, optionally code) and stage candidate business rules into `.specify/rules/_drafts/baseline-*.yml` for human review. Use this when adopting `byte-sized` on a brownfield project so the agent has an initial knowledge graph to navigate before any new spec is written.
...
1. **Parse flags** from `$ARGUMENTS` (all optional):
   ...
   - `--exclude "<csv-globs>"` appends to the default exclude list (`node_modules`, `.git`, `dist`, build outputs, the rules dir itself, etc.).
   - `--max-files-per-batch N` (default: 12).
   - `--max-bytes-per-batch N` (default: 65536, i.e. 64 KB).
   - `--dry-run` (default: false) — run discovery only, do not propose or stage rules.
...
4. **For each selected batch**, do the following sequentially (do **not** run batches in parallel — keeps token usage predictable):
   ...
   2. Propose candidate rules. Each candidate **must** be:
      ...
      Aim for **at most 5 candidates per batch** — quality over quantity. Prefer rules that are *not already obvious from the spec* (e.g. an enforcement detail buried in a config file, an unwritten convention visible only in commit hooks or CI).
   ...
   4. Pipe the YAML to the stage command:
      ...
      The stage call delegates to `rules-extract.sh` which de-duplicates against the existing index (statement token-overlap ≥ 0.7 → dropped) and writes survivors to `.specify/rules/_drafts/baseline-<batch>-<timestamp>.yml`.
...
5. **Emit the final summary**:
   ...
   - Dropped as duplicate of existing rules: <X>.

   Drafts staged:
     - .specify/rules/_drafts/baseline-b01-<ts>.yml  (<n1> rules)
     - .specify/rules/_drafts/baseline-b02-<ts>.yml  (<n2> rules)
     ...

   Next steps:
     1. Review each drafts file.
     2. Promote with `/speckit.byte-sized.add` (one per accepted rule) — this allocates the real id and runs the conflict pre-check.
...
- The discovery script never reads file contents itself — it returns the manifest, and the agent reads each batch via its own file tools. This keeps the script portable (no language-specific parsing) and lets the model judge what is rule-worthy.
- Code scope (`--scope code` or `all`) is **opt-in**. The default scope is `docs+specs` because most projects encode business rules in prose first; code-derived rules tend to be lower-signal and noisier.
...
The final summary block from step 5. Do not list every proposed rule — that volume defeats the token-efficiency contract. The user reviews the drafts files directly.
