...
# jq -c '.rules[]' returns one compact JSON object per line; PS captures them
...
$rulesJson = @(& jq -c '.rules[]' $index) | ForEach-Object { $_.Trim() } | Where-Object { $_ }
...
$allIds = @(& jq -r '.rules[].id' $index) | Where-Object { $_ } | Sort-Object -Unique
foreach ($r in $rulesJson) {
    if (-not $r) { continue }
    $id = ($r | & jq -r '.id').Trim()
