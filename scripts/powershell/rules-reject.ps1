# rules-reject.ps1 — reject one draft stub; move it into _drafts/_rejected/<basename>
# with audit fields ({ rejected: { at, reason } }) appended.
#
# Usage:
#   rules-reject.ps1 -FromFile <_drafts/foo.yml> -Index <N> [-Reason <text>]
#
...
if (-not (Test-Path $absFrom)) { throw "rules-reject: file not found: $absFrom" }

$stubsJson = & yq eval -o=json -I=0 '.' $absFrom
$type = ($stubsJson | & jq -r 'type').Trim()
if ($type -ne 'array') { throw "rules-reject: $absFrom is not a YAML array" }
$total = [int]($stubsJson | & jq 'length').Trim()
if ($Index -lt 0 -or $Index -ge $total) {
    throw "rules-reject: index $Index out of range (have $total)"
}
