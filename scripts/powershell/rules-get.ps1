...
for ($h = 0; $h -lt $Neighbors; $h++) {
    $hopFilter = '($idx[0].rules) as $all | ($seeds + ($all | map(select(.id as $i | $seeds | index($i))) | map(.edges // {}) | map([(.relates_to // []), (.depends_on // []), (.supersedes // []), (.conflicts_with // []), (.related_by_others // []), (.depended_on_by // []), (.superseded_by_others // []), (.conflicts_with_others // [])] | add) | add // [])) | unique'
