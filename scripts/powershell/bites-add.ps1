# bites-add.ps1 — create a new bite file from a YAML stub on stdin or flags.
...
$idPrefix = Get-BsCfg -Cfg $cfg -Path '.storage.id_prefix' -Default 'BB'
...
Invoke-BsWithLock -LockPath (Join-Path $bitesDir '.index.lock') -Action {
    ...
    if (-not $Statement -or -not $Domain) {
        throw "bites-add: -Statement and -Domain are required"
    }

    $domainUpper = $Domain.ToUpperInvariant()
    $domainLower = $Domain.ToLowerInvariant()
    $domainPath = Join-Path $bitesDir "domains/$domainLower"
    ...
    if (-not $Id) {
        ...
        if (Test-Path $index) {
            ...
            $maxFilter = '.bites | map(.id // "") | map(select(startswith($p))) | map(. | sub($p; "") | tonumber? // 0) | max // 0'
    ...
    if (Test-Path $index) {
        $dup = (& jq -r --arg id $Id '.bites | map(select(.id == $id)) | length' $index).Trim()
        if ($dup -ne '0') { throw "bites-add: id $Id already exists" }
    }
