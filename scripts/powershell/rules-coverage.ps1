...
foreach ($t in $tokens) {
    ...
    foreach ($p in $paths) {
        ...
        if (Select-String -Path $full -Pattern "\b$t\b" -Quiet) {
            ...
            if ($id) { $hits = & jq --arg id $id '. + [$id] | unique' --argjson h $hits -n '$h' }
        }
    }
    $coverage = & jq --arg t $t --argjson h $hits '. + {($t): $h}' --argjson c $coverage -n '$c'
}

$uncovered = & jq 'to_entries | map(select((.value | length) == 0)) | map(.key)' --argjson c $coverage -n '$c'
