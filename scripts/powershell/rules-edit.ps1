# rules-edit.ps1 — edit frontmatter fields of an existing rule in place.
#
# Usage:
#   rules-edit.ps1 -Id <BR-...> [-Statement <s>] [-RationaleFile <path>]
...
Invoke-BsWithLock -LockPath (Join-Path $rulesDir '.index.lock') -Action {
    ...
    if (-not $oldPath) { throw "rules-edit: $Id not found" }
    ...
    if ($hasRationale) {
        if (-not (Test-Path $RationaleFile)) { throw "rules-edit: rationale file not found: $RationaleFile" }
    ...
    if ($newDomain -ne $oldDomain) {
        ...
        if ($status -eq 'superseded') {
            $targetDir = Join-Path $rulesDir "_archive/$newDomain"
        } else {
            $targetDir = Join-Path $rulesDir "domains/$newDomain"
        }
