#!/usr/bin/env bash
# bites-add.sh — create a new bite file from a YAML stub on stdin or flags.
#
# Usage (flag form):
#   bites-add.sh --statement "..." --domain auth --rationale "..." \
#                --tags tag1,tag2 --status active --source-feature 002-foo \
#                --source-spec specs/002-foo/spec.md#FR-1 [--id BB-AUTH-007]
#
# Usage (stdin form):
#   cat candidate.yml | bites-add.sh --from-stdin
#
# Allocates the next id in the chosen domain unless --id is provided, writes the file,
# updates index.json, and emits the new bite id + path to stdout (JSON).
...
while (( $# > 0 )); do
  case "$1" in
    ...
    *) echo "bites-add: unknown arg: $1" >&2; exit 2 ;;
  esac
done
...
ID_PREFIX="$(bs_cfg "$CFG" '.storage.id_prefix' 'BB')"
...
if [[ -z "$STATEMENT" || -z "$DOMAIN" ]]; then
  echo "bites-add: --statement and --domain are required" >&2
  exit 2
fi

domain_upper="$(printf '%s' "$DOMAIN" | tr 'a-z' 'A-Z')"
domain_lower="$(printf '%s' "$DOMAIN" | tr 'A-Z' 'a-z')"
domain_path="$BITES_DIR/domains/$domain_lower"
mkdir -p "$domain_path"
...
if [[ -z "$ID" ]]; then
  ...
  if [[ -f "$BITES_DIR/index.json" ]]; then
    used="$(jq -r --arg p "${ID_PREFIX}-${domain_upper}-" '
      .bites | map(.id // "") | map(select(startswith($p))) | map(. | sub($p; "") | tonumber? // 0) | max // 0
    ' "$BITES_DIR/index.json")"
    next=$(( used + 1 ))
  fi
...
# Reject duplicate id.
if [[ -f "$BITES_DIR/index.json" ]] && \
   [[ "$(jq -r --arg id "$ID" '.bites | map(select(.id == $id)) | length' "$BITES_DIR/index.json")" != "0" ]]; then
  echo "bites-add: id $ID already exists" >&2
  exit 1
fi
