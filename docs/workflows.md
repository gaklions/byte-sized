...
The agent **never** loads the entire bites corpus. Every hook returns a *projection* — a JSON array of `{id, statement, domain, tags, status}` capped by `relevance.max_results` (default 12). The agent fetches a full bite body (`speckit.byte-sized.show <id>`) only when it decides it needs the rationale, examples, or neighbours to make a specific decision.
...
constitution  (no hook)
     │
     ▼
specify ──before──► surface  (compact list of bites whose tags/domain
     │              overlap with the seed prompt)
     │
     ▼  drafts the spec, citing relevant BB-* ids
     │
     ├──after──► extract  (optional: stage candidate bites to _drafts/)
     ▼
clarify
     │
     ├──after──► extract  (optional: stage bites captured from Q/A)
     ▼
...
3. Invokes `bites-query` with `--text <tokens> --domain <phase-hint> --status active --limit <max_results>`.
4. Emits a compact JSON block to the host command's context. Host commands are expected to cite matching bite ids using the format `[BB-AUTH-001]` in their generated artefacts.

## Capturing bites (`after_specify`, `after_clarify`)

`speckit.byte-sized.extract` is human-gated:

1. The agent reads the just-written spec/clarify file.
2. It proposes candidate bites (one-sentence statements + draft rationale + domain guess + tags).
3. Each candidate is written to `.specify/bites/_drafts/<feature>-<timestamp>.yml` as a YAML list.
4. The user reviews; approved drafts are promoted via `bites-add` (which allocates the real id, runs conflict pre-check, and updates `index.json`).
...
2. **Coverage** — for each `FR-*` in the active feature's spec/tasks, list the covering bite ids. Flag uncovered FRs and orphan bites.
3. **Conflict** — for each active bite, check same-domain bites with overlapping tags for lexical opposition markers. Flag candidate conflicts for human review.
...
3. For each accepted batch, the agent reads the files, proposes at most ~5 high-signal candidate bites (declarative, domain-tagged, evidence-cited), and pipes them to the same staging logic used by `extract`.
4. Drafts land in `.specify/bites/_drafts/baseline-<batch>-<ts>.yml` for human promotion via `/speckit.byte-sized.add`.
...
- `/speckit.byte-sized.add` — guided, conflict-checked bite creation.
- `/speckit.byte-sized.query <text>` — ad-hoc lookup from the agent.
- `/speckit.byte-sized.list` — human-facing summary.
- `/speckit.byte-sized.show BB-AUTH-001 --neighbors 2` — drill in.
- `/speckit.byte-sized.link BB-AUTH-001 depends_on BB-SEC-014` — manage edges.
